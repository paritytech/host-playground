...
// Wrapped in an async IIFE: Vite's worker bundle targets es2020 which doesn't
// allow top-level `await`. TypeScript accepts it under module: esnext, but the
// browser-target downlevel in esbuild rejects it.
(async () => {
  const chatManager = await getChatManager();
  if (!chatManager) {
    throw new Error("[echo-bot worker] getChatManager returned null - not inside a host container");
  }
